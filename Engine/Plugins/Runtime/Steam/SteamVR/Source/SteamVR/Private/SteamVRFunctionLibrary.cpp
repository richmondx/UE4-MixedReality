...
// MIXED-REALITY
void USteamVRFunctionLibrary::SetCaptureRenderTarget(UTextureRenderTarget2D * NewTexture, UTextureRenderTarget2D * NewTexture2)
{
	FSteamVRHMD* SteamVRHMD = GetSteamVRHMD();
	if (SteamVRHMD)
	{
		SteamVRHMD->SetCameraRenderTarget(NewTexture, NewTexture2);
	}
}

void USteamVRFunctionLibrary::RemoveCaptureRenderTarget(UTextureRenderTarget2D * NewTexture, UTextureRenderTarget2D * NewTexture2)
{
	FSteamVRHMD* SteamVRHMD = GetSteamVRHMD();
	if (SteamVRHMD)
	{
		SteamVRHMD->RemoveCameraRenderTarget(NewTexture, NewTexture2);
	}
}
